...
def read_file_from_cloud_storage(bucket_name, file_name, chunk_size=50000):
    logging.info("vinod inside the read_file_from_cloud_storage")
    storage_client = storage.Client()
    bucket = storage_client.get_bucket(bucket_name)
    blob = bucket.blob(file_name)
    logging.info("vinod blob creation")
    content = blob.download_as_text()
    logging.info("vinod after download as text")
    df = pd.read_csv(io.StringIO(content))
    logging.info("vinod read csv")
    df.columns = df.columns.str.strip()
    response_data_df, enrich_data_df = apply_validations(df)
    logging.info("vinod applu validation df")
